		...
		function redirect()
		{
			if(backgroundSlideOver===true && logoSlideOver===true){ /* la redirection ne se fait qu'après que les 2 animations soient terminées */
				if (team=='dinos') {
					window.location.replace('index.php?controller=deck&action=affichageDeck&hero=2');
				}
				if (team=='matrix') {
					window.location.replace('index.php?controller=deck&action=affichageDeck&hero=1');
				}
			}
		}